...
|---|---|---|---|
| 1 | **No authentication on any endpoint** — `/api/*`, `/ingest/*`, `/admin/*` are all open to anyone with the URL. CORS only restricts browser-based reads; it does nothing against a direct `curl`. | This is the actual reason the public site shows a "coming soon" placeholder instead of the real dashboard (see `DEPLOY.md`) — the data isn't safe to expose yet. | API keys or OAuth for `/ingest/*` and `/admin/*` at minimum; session/SSO auth in front of `/api/*` and the dashboard before it's the public-facing app. |
