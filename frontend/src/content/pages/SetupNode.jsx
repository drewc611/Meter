...
export const meta = {
  ...
  title: "Node setup — Merit",
  description:
    "Wire a Node backend's LLM calls into Merit's /ingest/usage endpoint, using the same proxy pattern as the Python reference — plus outcomes, quality signals, and verification.",
};
...
export default function SetupNode() {
  return (
    <ContentLayout active="setup" wide>
      ...
      <h1>Node setup</h1>
      <p className="lead">
        Merit's ingestion API is plain JSON over HTTPS, so a Node proxy follows the exact same
        ...
        Merit before returning.
      </p>

      <Toc
        items={[
          { href: "#usage", label: "The request that matters" },
          { href: "#fields", label: "Fields Merit expects" },
...
await fetch(\`\${MERIT_API_BASE}/ingest/usage\`, {
  ...
  body: JSON.stringify({
    ...
    tokens_out: usage.output_tokens,
      ...
      <p>
        Fire this after the response streams back to the caller (or via a queue) so a slow Merit
      ...
      <h2 id="fields">Fields Merit expects</h2>
      <p>
