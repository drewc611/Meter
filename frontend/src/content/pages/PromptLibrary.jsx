...
// complex-agent pattern ids, matching the ids/names in the AI system design
// patterns guide (/guides/ai-system-design-patterns), which is where the
// full diagrams and explanations of each one now live.
const PATTERN_NAMES = {
  "chatbot": "Chatbot / single-turn assistant",
  "rag": "Retrieval-augmented generation (RAG)",
  "copilot": "Coding assistant / copilot",
  "agent": "Agentic loop (tool-use agent)",
  "multi-agent": "Multi-agent / orchestrator",
  "router": "Router / classifier dispatch",
  "ensemble": "Ensemble / self-consistency",
  "evaluator": "Evaluator / LLM-as-judge",
  "memory-agent": "Memory-augmented agent",
  "fine-tuned": "Fine-tuned / specialized model",
  "workflow": "Workflow automation (LLM-in-pipeline)",
  "computer-use": "Computer-use / browser agent",
  "planner-executor": "Hierarchical planner-executor",
  "reflection": "Reflection / self-critique agent",
  "debate": "Debate / adversarial agents",
  "tree-of-thought": "Tree-of-thought exploration",
  "swarm": "Swarm / blackboard agents",
  "goal-stack": "Long-horizon goal-stack agent",
};

const PATTERNS_GUIDE_URL = "/guides/ai-system-design-patterns";
    ...
    <div className="card">
      ...
      <p className="composed-combines">
        ...
        {entry.combines.map((id, i) => (
          <Fragment key={id}>
            {i > 0 && ", "}
            <a href={`${PATTERNS_GUIDE_URL}#${id}`}>{PATTERN_NAMES[id]}</a>
          </Fragment>
        ))}
      </p>
...
export default function PromptLibrary() {
  return (
    <ContentLayout active="prompts" wide>
      ...
      <p className="lead">
        Two more prompt collections beyond the{" "}
        <a href="/prompts">daily 30-day archive</a> — prompts that combine multiple{" "}
        <a href="/guides/ai-system-design-patterns">AI system design patterns</a> for
        real, non-trivial work, each one naming exactly which patterns it's built from.
      </p>
      ...
      <h3>Healthcare &amp; clinical operations</h3>
      {ADVANCED_PROMPTS.healthcare.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Education &amp; academic administration</h3>
      {ADVANCED_PROMPTS.education.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Logistics &amp; distribution operations</h3>
      {ADVANCED_PROMPTS.logistics.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Energy &amp; utilities operations</h3>
      {ADVANCED_PROMPTS.energy.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Media, publishing &amp; content operations</h3>
      {ADVANCED_PROMPTS.media.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Public sector &amp; government operations</h3>
      {ADVANCED_PROMPTS.publicsector.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Manufacturing &amp; industrial operations</h3>
      {ADVANCED_PROMPTS.manufacturing.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Real estate &amp; construction</h3>
      {ADVANCED_PROMPTS.realestate.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Insurance underwriting &amp; claims</h3>
      {ADVANCED_PROMPTS.insurance.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Nonprofit &amp; grant-funded programs</h3>
      {ADVANCED_PROMPTS.nonprofit.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Game studios &amp; live-service operations</h3>
      {ADVANCED_PROMPTS.gaming.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Biotech R&amp;D &amp; clinical trials</h3>
      {ADVANCED_PROMPTS.biotech.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Retail &amp; e-commerce operations</h3>
      {ADVANCED_PROMPTS.retail.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Telecom &amp; network operations</h3>
      {ADVANCED_PROMPTS.telecom.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Sports teams &amp; athlete operations</h3>
      {ADVANCED_PROMPTS.sports.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}

      <h3>Regulatory operations across industries</h3>
      {ADVANCED_PROMPTS.regops.map((entry) => (
        <ComposedPromptCard entry={entry} key={entry.title} />
      ))}
    </ContentLayout>
  );
