...
description: >-
  A map of the fourteen domains that make up an enterprise agentic DevSecOps
  platform, from platform operating model to GovCloud, adapted from our own
  reference handbook.
kicker: Guide · from the Enterprise Agentic DevSecOps Handbook
badge: From the Merit AC team
lead: >-
  The ten control disciplines repeat throughout an enterprise deployment — this
  is the map of where. Fourteen domains, each a real chapter of our own
  Enterprise Agentic DevSecOps Handbook, from the platform's operating model
  down to the technical reference at the back.
wide: true
group: devsecops
tileMeta: >-
  A map from platform operating model to GovCloud, with the service reference
  table
---
...
> **Tiering example:** repository reads, searches, unit test execution, and `terraform plan` can run unattended. IAM policy changes, production deploys, secrets rotation, destructive operations, and any policy exception require a named approver who sees the diff, the policy result, and the rollback plan before authorizing.
...
>
> *   Confirmed service and model availability in the target GovCloud region
> *   Network path: VPC endpoints, egress rules, no unreviewed internet reachability
> *   KMS key policy and regional key isolation for Memory and stored evidence
> *   Data residency and log retention matched to the compliance regime
...
| Service / capability | Role in this platform | Control focus |
| --- | --- | --- |
...
*   AWS Prescriptive Guidance: Core services — tools
*   AWS Architecture Icons: official AWS-approved icon toolkit

See [the 30-day challenge](/challenge) for a day-by-day path through these domains, or [Four control boundaries](/guides/four-control-boundaries) for the short version of why this split exists at all.
